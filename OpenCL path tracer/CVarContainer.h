...
public:
	...
	template<typename T> T& create_cvar(std::string name, T& var, std::string help = "");		// Creates a CVar and stores it, while returns a reference to the variable
	...
	template<typename T> void attach_cvar(std::string name, T* var, std::string help = "");		// Attaches a variable to a CVar
...
template<typename T> T& CVarContainer::create_cvar(std::string name, T& var, std::string help) {
	if (find(name, false) == NULL) {					// If CVar does not exist
		CVar<T>* cvar = new CVar<T>(name, var, help);	// Create it
...
template<typename T> void CVarContainer::attach_cvar(std::string name, T* var, std::string help) {
	CVarBase* cvarnode = find(name, false);				// Try to find CVar
	if (cvarnode != NULL) {								// If found
		CVar<T>* cvar = (CVar<T>*)cvarnode;				// Get it's value
		cvar->set_var_p(var);							// Set the pointer that points to the new data
	} else {
		this->create_cvar(name, *var, help);			// Else create a CVar
	}
}
